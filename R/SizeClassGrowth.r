...
SizeClassGrowth<-function(xdataset, method="median",dbh ="D4"){
	...
	AGBData$Delta.time<-AGBData$Census.Mean.Date-AGBData$Census.prev
   	Class1<-AGBData[!is.na(AGBData[,dbh]) & AGBData[,dbh]>=50 & AGBData[,dbh]<200,]
	Class2<-AGBData[!is.na(AGBData[,dbh]) & AGBData[,dbh]>=200 & AGBData[,dbh]<400,]
	Class3<-AGBData[!is.na(AGBData[,dbh]) & AGBData[,dbh]>=400,]
